// src/app/[locale]/procurement/page.tsx
...
interface IProcurementItem {
  ...
  // ИСПРАВЛЕНИЕ: Указываем, что это массив файлов или null
...
async function getProcurements(locale: string) {
  try {
    const strapiResponse = await fetchApi<{ 
      data: IProcurementItem[], 
      meta: any 
    }>('/api/procurements', locale, {
...
export default async function ProcurementPage() {
  ...
  const t = await getTranslations('ProcurementPage');
  const procurements = await getProcurements(locale);
          ...
          <ul className="divide-y">
            {procurements.length > 0 ? (
              procurements.map((item) => {
                // --- ИСПРАВЛЕНИЕ: Извлекаем первый файл из массива ---
                const docArray = item.documentFile;
                const doc = docArray && docArray.length > 0 ? docArray[0] : null;

                if (!doc) return null; // Пропускаем, если файла нет
